...
public abstract class MyAbstractQueuedSynchronizer
    ...
    /**
     * 将当前线程放到等待队列，并，让其阻塞，并，自旋消费等待队列里面的节点让自己尝试获取锁
    ...
    /**
     * 将当前线程放到等待队列，并，让其阻塞，并，自旋消费等待队列里面的节点让自己尝试获取锁（注意：该线程可以被打断）
    ...
    /**
     * 将当前线程放到等待队列，并，让其阻塞，并，自旋消费等待队列里面的节点让自己尝试获取锁（注意：该线程可以被打断，可以等待超时）
    ...
    public final void acquire(int arg) {
    	// 如果获取锁失败 并且 将当前线程放到等待队列，并，让其阻塞，并，自旋消费等待队列里面的节点让自己尝试获取锁
    	// 注意：Node.EXCLUSIVE 表示独占锁节点
     ...
     *        can represent anything you like.（要减掉重入的次数）
    ...
    public final boolean release(int arg) {
        ...
        if (tryRelease(arg)) {
            Node h = head;
            // 如果等待锁的节点不为空，并且处于等待状态，就唤醒该节点的线程
